...
export const OfferSpotlightBlock: React.FC<Props> = ({
  ...
  return (
    // Always a full-width cobalt panel (like the "Our Promise" band). The Padding above/below
    // controls the space *inside* the card — "Default" is the medium size; pick Small/Extra
    // small (or None) to tighten it, Large to open it up.
    <SectionShell
      surface="brandPanel"
        ...
        <div className="relative flex justify-center lg:border-l lg:border-dashed lg:border-white/25 lg:pl-14">
          <div className="relative grid size-48 rotate-[-7deg] place-items-center rounded-full bg-gold text-gold-foreground sm:size-60">
            ...
            <div className="flex flex-col items-center gap-2 px-8 text-center">
              ...
              {seal && (
                <span className="text-xs font-bold uppercase leading-tight tracking-[0.12em]">{seal}</span>
              )}
            </div>
          </div>
        </div>
